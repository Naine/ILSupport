...
namespace ILSupport
{
    public static partial class ILParser
    {
        public static string[] GetWords(string @class)
        {
            switch (@class)
            {
            case ClassIlInstruction:
                return ilInstructions;
            case PredefinedClassificationTypeNames.Keyword:
                return ilKeywords;
            case ClassIlDirective:
                return ilDirectives;
            case ClassIlSecurity:
                return ilSecurityWords;
            }
            return null;
        }
        ...
        public static string IdentifyWordClass(string word)
        {
            ...
            if (wordClasses.TryGetValue(word, out string cls))
                return cls;

            return null;
        }

        private const string ClassIlInstruction = "il.instruction";
        private const string ClassIlDirective = "il.directive";
        private const string ClassIlSecurity = "il.security";

        private static readonly string[] ilInstructions = {
            "nop", "break",
        ...
        private static readonly string[] ilKeywords = {
            "void", "bool", "char", "wchar", "int", "int8", "int16", "int32", "int64", "uint8", "uint16", "uint32", "uint64", "float", "float32", "float64",
        ...
        private static readonly string[] ilDirectives = {
        ...
        private static readonly string[] ilSecurityWords = {
        ...
        private static readonly Dictionary<string, string> wordClasses = BuildClassDict();

        private static Dictionary<string, string> BuildClassDict()
        {
            var d = new Dictionary<string, string>();
            foreach (var word in ilInstructions)
            {
                d.Add(word, ClassIlInstruction);
            }
            foreach (var word in ilKeywords)
            {
                d.Add(word, PredefinedClassificationTypeNames.Keyword);
            }
            foreach (var word in ilDirectives)
            {
                d.Add(word, ClassIlDirective);
            }
            foreach (var word in ilSecurityWords)
            {
                d.Add(word, ClassIlSecurity);
            }
            return d;
        }
    }
